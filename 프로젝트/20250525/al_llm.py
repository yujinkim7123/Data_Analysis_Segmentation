...
# OpenAI 클라이언트
client = OpenAI(api_key="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")  # 실제 키로 대체
...
def search_context(keywords, collection, top_k=3, min_score=0.75):
    results = []
    for kw in keywords:
...
if user_input:
    with st.spinner("페르소나 생성 중..."):
        ...
        # 2. 문장 검색
        context_web = search_context(expanded, "web_data")
        context_product = search_context(expanded, "product_feature_data")
